...
load('main.df')
load('South_Canceled_classes.Rdata')
...
for (i in 1:nrow(df[df$college=='064',])){
  if(df$code[df$college=='064'][i] %in% 
      names(cancel_short_list)){
...
for (i in 1:nrow(df[df$college=='064',])){
  if(df$code[df$college=='064'][i] %in% 
     names(cancel_short_list)){
    for (j in 1:length(paste(cancel_short_list,'$',df$code)))
    {if(df$Item[i] %in% cancel_short_list[[j]])
      print}
    print(df$code[i])
...
for ( i in 1:length(cancel_short_list)){
  if(main.df$code==cancel_short_list[i])
    {print(main.df$site_enrol)}
}

...
rows[i] <- ( nrow(main.df[,"scrap_tbl"][[i]]))
}

main <- matrix(ncol=1,nrow=sum(rows))
main <- data.frame(main)
datalist = list()
for (i in 1:length(rows)){
prelim.df <- cbind(rep_len(main.df[,2:5][i],length.out = rows[i]))
  datalist[[i]]<- cbind(prelim.df,main.df$scrap_tbl[[i]])
}

test<- (do.call(rbind, datalist))

nrow(test)
```
test[grep('^bholt',test$Instructor,ignore.case=T),] %>% summarise(mean(Enrolled))

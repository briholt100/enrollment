...
df %>% select(college,code,Season,yr,Enrolled) %>% 
  ...
  mutate(Date = ymd(Date)) %>% filter(Season!="Summer") %>% 
  group_by(college,Date,Season) %>% summarise(Total_Enrol=sum(Enrolled)) %>% 
  ggplot(aes(x=Date,y=Total_Enrol,color=Season))+geom_line()+facet_wrap(~college)+ggtitle("Total Enrollment over time")
